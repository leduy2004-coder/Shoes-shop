...
public class UserVariantService {
        ...
        public PageResponse<UserPurchasedItemResponse> getPurchasedByUser(String userId, int page, int size) {
                ...
                List<UserPurchasedItemResponse> items = userVariants.stream()
                                .map(uv -> {
                                        VariantSizeEntity variantSize = variantSizeRepository
                                                        .findById(uv.getVariantSizeId())
                                                        .orElse(null);
                                        if (variantSize == null) {
                                        ...
                                        ProductGetResponse productDto = modelMapper.map(product,
                ...
                Map<String, CloudinaryResponse> imageCache = batchLoadProductImages(
                                items.stream()
                                                .map(item -> item.getProduct() != null ? item.getProduct().getId()
                                                                : null)
                                                .filter(Objects::nonNull)
                                                .distinct()
        ...
        public PageResponse<UserPurchasedOrderResponse> getPurchasedOrdersByUser(String userId, int page, int size) {
                ...
                Sort sort = Sort.by(Sort.Direction.DESC, "createdDate");
                Pageable pageable = PageRequest.of(Math.max(0, page - 1), Math.max(1, size), sort);
                Page<PurchaseOrderEntity> ordersPage = purchaseOrderRepository.findByUserIdAndStatus(userId, true,
                                pageable);
                ...
                Map<String, VariantSizeEntity> variantSizeMap = variantSizeIds.isEmpty()
                                ...
                                : variantSizeRepository.findAllById(variantSizeIds).stream()
                                                .collect(Collectors.toMap(VariantSizeEntity::getId,
                                                                Function.identity()));
                ...
                List<UserPurchasedOrderResponse> orderResponses = orders.stream()
                                .map(order -> {
                                        ...
                                        List<ItemOrderResponse> items = userVariants.stream()
                                                        .map(uv -> {
                                                                VariantSizeEntity variantSize = variantSizeMap
                                                                                .get(uv.getVariantSizeId());
                                                                if (variantSize == null)
                                                                        return null;

                                                                VariantEntity variant = variantMap
                                                                                .get(variantSize.getVariantId());
                                                                if (variant == null)
                                                                        return null;

                                                                ProductEntity product = productMap
                                                                                .get(variant.getProductId());
                                                                if (product == null)
                                                                        return null;
                                                                ...
                                                                // Set image URL
                                                                CloudinaryResponse image = imageCache
                                                                                .get(product.getId());
                                                                if (image != null) {
                                        ...
                                        return UserPurchasedOrderResponse.builder()
                                                        ...
                                                        .addressId(null) // Không còn dùng addressId nữa, address được
                                                                         // lưu trực tiếp trong order
        ...
        public PageResponse<UserPurchasedItemResponse> getPurchasedByProductId(String productId, int page, int size) {
                ...
                Sort sort = Sort.by(Sort.Direction.DESC, "createdDate");
                Pageable pageable = PageRequest.of(Math.max(0, page - 1), Math.max(1, size), sort);
                Page<UserVariantEntity> userVariantsPage = userVariantRepository
                                .findByVariantSizeIdInAndStatus(variantSizeIds, true, pageable);
                List<UserVariantEntity> userVariants = userVariantsPage.getContent();
                                ...
                                .collect(Collectors.toMap(
                                                ...
                                                userId -> {
                                                        try {
                                                                ...
                                                                log.warn("Could not fetch profile for userId: {}",
                                                                                userId, e);
                                                                return null;
                                                        }
                                                }));
        ...
        public OrderDetailResponse getOrderDetail(String userVariantId) {
                ...
                Map<String, VariantSizeEntity> variantSizeMap = variantSizeRepository.findAllById(variantSizeIds)
                                .stream()
                                .collect(Collectors.toMap(VariantSizeEntity::getId, Function.identity()));
        ...
        private Map<String, CloudinaryResponse> batchLoadProductImages(List<String> productIds) {
                ...
                // Load images sequentially to ensure RequestContext is available for Feign
                // Interceptor
